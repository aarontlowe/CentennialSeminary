...
exports.handler = async (event) => {
  ...
  const html = `<h1>Seminary Makeup Work Submission</h1><p><strong>Student Name:</strong> ${escapeHtml(payload.studentName)}</p><p><strong>Student Email:</strong> ${escapeHtml(payload.studentEmail)}</p><p><strong>Class:</strong> ${escapeHtml(payload.classPeriod)}</p><p><strong>Lesson:</strong> ${escapeHtml(payload.lessonWeek)}</p><h2>What the student learned</h2><p>${escapeHtml(payload.learningResponse).replace(/\n/g,"<br>")}</p><h2>How this lesson helps the student come closer to Jesus Christ</h2><p>${escapeHtml(payload.christResponse).replace(/\n/g,"<br>")}</p>`;
